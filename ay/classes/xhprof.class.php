...
class XHProf
{
	...
	/**
	 * param	array	$callstack	Raw callstack. Passing aggregated callstack will produce unexpected results.
	 */
	public function assignUID($callstack)
	{
...
class XHProfCallgraph
{
	...
	public function dot($callstack, $output = FALSE, $debug = FALSE)
	{
		...
		if(!isset($mother['uid']))
		{
			throw new XHProfException('Invalid callstack input. UIDs are not populated.');
		}
		...
		foreach($callstack as $e)
		{
			...
			{
				throw new XHProfException('Duplicate player is not possible in an exclusive callstack.');
			}
			...
			else
			{
				...
				<table border="0" cellspacing="0" cellborder="1" cellpadding="5">
					<tr>
						<td colspan="2" align="left">' . $e['callee'] . '</td>
					</tr>
					' . $ct . '
					<tr>
						<td align="left">wt</td>
	...
	public function graph($dot_script)
	{
		...
		if(!empty($error))
		{
			throw new XHProfException('DOT produced an error.');
		}
		
		if(empty($output))
		{
			throw new XHProfException('DOT did not output anything.');
		}
